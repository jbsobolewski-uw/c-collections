...
static int edge(void) {
    ...
    for (uint32_t i = 0; i < 5; ++i) ASSERT(hm_get(map, i) == &values[i]);
...
static int collisions(void) {
    ...
    for (uint32_t k = 0; found < COLLIDING_KEYS && k < 100000; ++k)
        if ((size_t) test_hash(k) % TEST_CAPACITY == slot) keys[found++] = k;
    ...
    for (size_t i = 0; i + 1 < COLLIDING_KEYS; ++i)
        if (i != 2) ASSERT(hm_get(map, keys[i]) == &values[i]);
...
static int shrink(void) {
    ...
    ASSERT(mtd->alloc_counter >
           allocs_before); /* shrinking actually rehashed */
    ...
    for (uint32_t i = 0; i < 500; ++i) ASSERT(hm_get(map, i) == &values[i]);
...
static unsigned long scenario(void) {
    ...
    errno = 0;
    if ((map = hm_create(64, free, 0)) != NULL) visited |= V(1, 0);
    ...
    for (uint32_t key = 0; key < 50; ++key) {
        ...
        *val = (int) key + 1000;
        ...
        errno = 0;
        if (hm_insert(map, key, val) == HASHMAP_OK) visited |= V(1, where);
    ...
    if (hm_size(map, &size) == HASHMAP_OK && size == 50) visited |= V(1, 11);
    else {
        hm_destroy(map);
    ...
    if (hm_destroy(map) == HASHMAP_OK) visited |= V(1, 15);
    else visited |= V(4, 15);
...
static const test_list_t test_list[] = {
        TEST(api),    TEST(edge),   TEST(ownership), TEST(collisions),
        TEST(growth), TEST(shrink), TEST(memory),
};
