...
static int stress(void) {
  enum { WAVES = 8, ENQ_PER_WAVE = 3000, DEQ_PER_WAVE = 1500 };
  ...
  for (int w = 0; w < WAVES; ++w) {
    for (int i = 0; i < ENQ_PER_WAVE; ++i) {
      ASSERT(queue_enqueue(q, (void *) (uintptr_t) (next_in + 1)) == QUEUE_OK);
      ++next_in;
    }
...
static unsigned long scenario(void) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    errno = 0;
    if ((p[i] = (int *) malloc(sizeof *p[i])) != NULL)
      visited |= V(1, mw);
    else if (errno == ENOMEM && (p[i] = (int *) malloc(sizeof *p[i])) != NULL)
      visited |= V(2, mw);
    else {
      queue_destroy(q);
    ...
    *p[i] = (int) i;
    ...
    errno = 0;
    if (queue_enqueue(q, p[i]) == QUEUE_OK)
      visited |= V(1, ew);
  ...
  errno = 0;
  if ((p[3] = (int *) malloc(sizeof *p[3])) != NULL)
    visited |= V(1, 12);
  ...
  *p[3] = 3;
  ...
  errno = 0;
  if (queue_enqueue(q, p[3]) == QUEUE_OK)
    visited |= V(1, 13);
