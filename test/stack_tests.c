...
static int api(void) {
  ...
  ASSERT(log.count == N);
  for (size_t i = 0; i < N; ++i)
    ASSERT(log.values[i] == *elems[N - 1 - i]);
...
static unsigned long scenario(void) {
  ...
  for (size_t i = 0; i < SIZE(e); ++i) {
    ...
    *e[i] = 1 << i;
    ...
    errno = 0;
    if (stack_push(s, e[i]) == STACK_OK)
      visited |= V(1, push_where);
  ...
  errno = 0;
  if ((late = malloc(sizeof *late)) != NULL)
    visited |= V(1, 9);
