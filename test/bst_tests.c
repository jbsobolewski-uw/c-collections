...
#define EXPECT_ERRNO(call, e)                                                  \
    do {                                                                       \
        errno = 0;                                                             \
        if ((call) != BST_ERR || errno != (e)) return FAIL;                    \
    } while (0)
...
static int item_cmp(void *a, void *b) {
    ...
    if (x->key < y->key) return BST_LE;
    if (x->key > y->key) return BST_GR;
    return BST_EQ;
}
...
static int tree_matches(bst_t *t, long const *keys, size_t n) {
    size_t sz = 0;
    if (bst_size(t, &sz) != BST_OK || sz != n) return 0;
    for (size_t i = 0; i < n; ++i) {
        long  k   = keys[i];
        void *out = NULL;
...
static int api(void) {
    ...
    /* Removal of all three node shapes, non-root and root. */
    {
        ...
        ASSERT(remove_shape(i2, SIZE(i2), 5, r2,
                            SIZE(r2))); /* left child only */

        static long const i3[] = {10, 15, 20}, r3[] = {10, 20};
        ASSERT(remove_shape(i3, SIZE(i3), 15, r3,
                            SIZE(r3))); /* right child only */
        ...
        ASSERT(remove_shape(i6, SIZE(i6), 10, r6,
                            SIZE(r6))); /* root, left only */

        static long const i7[] = {10, 15}, r7[] = {15};
        ASSERT(remove_shape(i7, SIZE(i7), 10, r7,
                            SIZE(r7))); /* root, right only */

        static long const i8[] = {10, 5, 15}, r8[] = {5, 15};
        ASSERT(remove_shape(i8, SIZE(i8), 10, r8,
                            SIZE(r8))); /* root, two children */
...
static int ownership(void) {
    ...
    /* NULL destructor: the tree never touches the elements. */
    {
        ...
        for (size_t i = 0; i < SIZE(p); ++i) free(p[i]);
    }
     ...
     * duplicated or dropped. */
    {
        ...
        ASSERT(bst_destroy(t) == BST_OK);
        for (size_t i = 0; i < SIZE(dup_elems); ++i) ASSERT(dup_counts[i] == 1);
...
static unsigned long scenario(void) {
    ...
    errno = 0;
    if ((t = bst_create(NULL, free)) != NULL) visited |= V(1, 0);
    ...
    for (unsigned i = 0; i < 4; ++i) {
        ...
        errno = 0;
        if ((p = (long *) malloc(sizeof *p)) != NULL) visited |= V(1, w);
        ...
        *p = keys[i];
        ...
        errno = 0;
        if (bst_insert(t, p) == BST_OK) visited |= V(1, w + 1);
    ...
    size_t sz = 0;
    errno     = 0;
    if (bst_size(t, &sz) == BST_OK && sz == 4) visited |= V(1, 10);
    else {
        bst_destroy(t);
    ...
    errno = 0;
    if (bst_remove(t, &k) == BST_OK) visited |= V(1, 11);
    else {
        bst_destroy(t);
    ...
    errno = 0;
    if ((p = (long *) malloc(sizeof *p)) != NULL) visited |= V(1, 12);
    else if (errno == ENOMEM && (p = (long *) malloc(sizeof *p)) != NULL)
        visited |= V(2, 12);
    else {
        bst_destroy(t);
    ...
    *p = 15;
    ...
    errno = 0;
    if (bst_insert(t, p) == BST_OK) visited |= V(1, 13);
    else if (errno == ENOMEM && bst_insert(t, p) == BST_OK) visited |= V(2, 13);
    else {
        free(p);
        bst_destroy(t);
    ...
    int empty = -1;
    errno     = 0;
    if (bst_is_empty(t, &empty) == BST_OK && empty == 0) visited |= V(1, 14);
    else {
        bst_destroy(t);
    ...
    errno = 0;
    if (bst_remove(t, &k) == BST_OK) visited |= V(1, 15);
    else {
        bst_destroy(t);
    ...
    errno = 0;
    if (bst_size(t, &sz) == BST_OK && sz == 3) visited |= V(1, 16);
    else {
        bst_destroy(t);
