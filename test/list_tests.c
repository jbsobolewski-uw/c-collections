...
static unsigned long scenario(void) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    errno = 0;
    if ((e[i] = malloc(sizeof *e[i])) != NULL)
      visited |= V(1, where_elem);
    ...
    *e[i] = i;
    ...
    errno = 0;
    if (list_add(l, e[i]) == LIST_OK)
      visited |= V(1, where_add);
  ...
  *e[4] = 4;
  ...
  int empty = 1;
  if (list_is_empty(l, &empty) == LIST_OK && empty == 0)
    visited |= V(1, 14);
  else {
    (void) list_destroy(l);
